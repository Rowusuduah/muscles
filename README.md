...
- Equipment-led gym visits start directly from any verified machine guide: choose a linked movement, log the working sets, return to Equipment for the next station, then finish and save the whole visit. An unfinished visit is restored after closing the app, partner/open-gym visits never advance the solo program rotation, and multiple same-day sessions are preserved instead of overwritten.
